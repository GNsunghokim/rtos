...
local function insertFiles(device, files, hook)
    local dir = {}
    ...
    local inserter = {
        ...
        hook,
        'sudo cp ' .. table.concat(files, ' ')  .. ' mnt',
        'sync'
    }
...
local function buildSystemImage(loader, kernel, ramdisk)
    ...
    local device = execute('sudo losetup -f')
    os.execute('sudo kpartx -asv ' .. image)
    ...
    local files = {
        loader, kernel, ramdisk,
        'bin/grub.cfg',
    }
    local hook = 'sudo mkdir -p mnt/boot/grub/i386-pc'
    if not burnImage(bootDevice, 'vfat', files, hook) then
        print('\tFailed to copy files to emtpy ramdisk image : ' .. table.concat(files, ' '))
